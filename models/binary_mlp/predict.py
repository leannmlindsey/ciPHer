"""Prediction interface for trained BinaryOneVsRestMLP models.

Loads trained K and O heads (each = a shared trunk + per-class binary heads)
and exposes the standard cipher Predictor interface for cipher-evaluate.
Originally branched from models/attention_mlp/predict.py — only the model
class name and the per-class config keys (trunk_dims / head_hidden /
use_input_batchnorm) differ.
"""
...
from model import BinaryOneVsRestMLP


class BinaryOneVsRestMLPPredictor(Predictor):
    """Predictor that uses trained K and O BinaryOneVsRestMLP heads."""
...
def _load_head(model_dir, device):
    ...
    model = BinaryOneVsRestMLP(
        input_dim=config['input_dim'],
        num_classes=config['num_classes'],
        trunk_dims=config['trunk_dims'],
        head_hidden=config['head_hidden'],
        dropout=config['dropout'],
        use_input_batchnorm=config.get('use_input_batchnorm', True),
    )
...
def get_predictor(experiment_dir):
    ...
    Returns:
        BinaryOneVsRestMLPPredictor instance
    """
    ...
    return BinaryOneVsRestMLPPredictor(
        k_model=k_model,
        o_model=o_model,
