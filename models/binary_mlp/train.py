"""Training script for BinaryOneVsRestMLP — one binary classifier per serotype.
...
from model import BinaryOneVsRestMLP, SerotypeDataset, get_loss_fn, compute_metrics
...
def train_head(head_name, X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    trunk_dims = model_cfg.get('trunk_dims', [1024, 256])
    head_hidden = model_cfg.get('head_hidden', 64)
    use_input_batchnorm = model_cfg.get('use_input_batchnorm', True)
    ...
    # Create model — shared trunk + per-class binary heads.
    ...
    model = BinaryOneVsRestMLP(
        ...
        dropout=dropout,
        use_input_batchnorm=use_input_batchnorm,
    )
...
def train(experiment_dir, config):
    """Train both K and O heads for the BinaryOneVsRestMLP model.
    ...
    print('=' * 70)
    print('TRAINING BinaryOneVsRestMLP')
    print('=' * 70)
